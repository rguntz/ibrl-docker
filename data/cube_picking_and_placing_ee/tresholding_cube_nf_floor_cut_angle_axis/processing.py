...
def quat_to_angle_axis(quats):
    ...
    Input: (N, 4) with w first (MuJoCo convention)
    Output: (N, 3) rotvec
    """
    if quats.shape[-1] != 4:
        raise ValueError(f"Expected quat shape (..., 4), got {quats.shape}")
...
def switch_to_angle_axis(input_path, output_path):
    """
    Convert quaternions to angle-axis in a dataset where:
      - obs['robot0_eef_quat'] is (T, 8): [left_quat (4), right_quat (4)]
      - actions are (T, 16): [l_pos(3), l_quat(4), l_grip(1), r_pos(3), r_quat(4), r_grip(1)]
    
    Output:
      - obs['robot0_eef_aa']: (T, 6) = [left_aa(3), right_aa(3)]
      - actions: (T, 14) = [l_pos(3), l_aa(3), l_grip(1), r_pos(3), r_aa(3), r_grip(1)]
    """
    input_path = Path(input_path)
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with h5py.File(input_path, "r") as fin, h5py.File(output_path, "w") as fout:
        ...
        for demo_name in data_in.keys():
            ...
            # Copy all observation keys except 'robot0_eef_quat'
            for key in obs_in.keys():
                if isinstance(obs_in[key], h5py.Dataset):
                    obs_out.create_dataset(key, data=obs_in[key][:], compression="gzip")
                else:
                    obs_in.copy(key, obs_out)

            # Process robot0_eef_quat: (T, 8) → split → convert → recombine to (T, 6)
            if "robot0_eef_quat" not in obs_in:
                raise KeyError(f"'robot0_eef_quat' missing in {demo_name}")

            quat_full = obs_in["robot0_eef_quat"][:]  # (T, 8)
            T = quat_full.shape[0]
            if quat_full.shape[1] != 8:
                raise ValueError(f"Expected robot0_eef_quat shape (T, 8), got {quat_full.shape}")

            left_quat = quat_full[:, 0:4]   # (T, 4)
            right_quat = quat_full[:, 4:8]  # (T, 4)

            left_aa = quat_to_angle_axis(left_quat)    # (T, 3)
            right_aa = quat_to_angle_axis(right_quat)  # (T, 3)

            eef_aa = np.concatenate([left_aa, right_aa], axis=1)  # (T, 6)
            obs_out.create_dataset("robot0_eef_aa", data=eef_aa, compression="gzip")

            # --- Process actions ---
            if "actions" not in demo_in:
                raise KeyError(f"'actions' missing in {demo_name}")

            actions_old = demo_in["actions"][:]  # (T, 16)
            if actions_old.shape[1] != 16:
                raise ValueError(f"Expected action dim 16, got {actions_old.shape[1]}")

            # Left arm: indices 0-7 → [0:3=pos, 3:7=quat, 7:8=grip]
            l_pos = actions_old[:, 0:3]
            l_quat = actions_old[:, 3:7]
            l_grip = actions_old[:, 7:8]

            # Right arm: indices 8-15 → [8:11=pos, 11:15=quat, 15:16=grip]
            r_pos = actions_old[:, 8:11]
            r_quat = actions_old[:, 11:15]
            r_grip = actions_old[:, 15:16]

            # Convert quats to angle-axis
            l_aa = quat_to_angle_axis(l_quat)  # (T, 3)
            r_aa = quat_to_angle_axis(r_quat)  # (T, 3)

            # Rebuild action: [l_pos(3), l_aa(3), l_grip(1), r_pos(3), r_aa(3), r_grip(1)] = 14D
            actions_new = np.concatenate([l_pos, l_aa, l_grip, r_pos, r_aa, r_grip], axis=1)

            demo_out.create_dataset("actions", data=actions_new, compression="gzip")

            # --- Copy other top-level keys (rewards, dones, etc.) ---
            for key in demo_in.keys():
                if key in ("obs", "actions"):
                    continue
                item = demo_in[key]
                if isinstance(item, h5py.Dataset):
                    demo_out.create_dataset(key, data=item[:], compression="gzip")
                else:
                    demo_in.copy(key, demo_out)

        print(f"✅ Quaternion → Angle-Axis conversion complete!")
        print(f"  - obs['robot0_eef_quat'] (T,8) → obs['robot0_eef_aa'] (T,6)")
        print(f"  - actions (T,16) → (T,14)")
...
def inspect_right_arm_angle_axis(file):
    with h5py.File(file, "r") as f:
        ...
        # Actions — always 14D: [left(7), right(7)]
        action = f_demo_0["actions"][:]
        if action.shape[1] != 14:
            raise ValueError(f"Expected action dimension 14 (bimanual angle-axis), but got {action.shape[1]}")
        print("Action size:", action.shape)
        print("First action:", action[0])
        print("Last action:", action[-1])
        ...
        # --- Extract right arm proprioception ---
        # Position: last 3 of robot0_eef_pos → right arm
        robot0_eef_pos = obs["robot0_eef_pos"][:, 3:]  # (T, 3)

        # Angle-axis: last 3 of robot0_eef_aa → right arm
        if "robot0_eef_aa" not in obs:
            raise KeyError("Observation missing 'robot0_eef_aa'. Make sure you ran switch_to_angle_axis().")
        robot0_eef_aa = obs["robot0_eef_aa"][:, 3:]    # (T, 3)

        # Gripper: second column → right gripper
        robot0_gripper_qpos = obs["robot0_gripper_qpos"][:, 1:]  # (T, 1)

        # Concatenate into 7-D right arm observation
        right_arm_obs = np.concatenate([robot0_eef_pos, robot0_eef_aa, robot0_gripper_qpos], axis=1)  # (T, 7)

        # Right arm action: last 7 dimensions of 14D action
        right_arm_action = action[:, 7:]  # (T, 7)

        T = right_arm_obs.shape[0]
        if T != right_arm_action.shape[0]:
            print(f"⚠️ Warning: obs steps ({T}) != action steps ({right_arm_action.shape[0]})")

        steps = range(T)

        # --- Plot all 7 dimensions: pos (3), aa (3), gripper (1) ---
        dim_names = ["EEF X", "EEF Y", "EEF Z", "AA X", "AA Y", "AA Z", "Gripper"]
        ...
        axes[-1].set_xlabel("Step")
        plt.suptitle("Right Arm: Full State (Pos + Angle-Axis + Gripper) vs Action")
        plt.tight_layout(rect=[0, 0, 1, 0.97])
        plt.show()
...
######################################################################################################################################################################################
file_original = "/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1.hdf5"
inspect_right_arm(file_original)
normalize_gripper_in_file(input_path=file_original, output_path="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper.hdf5")
inspect_right_arm(file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper.hdf5")
process_dataset(input_file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper.hdf5", output_file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded.hdf5", threshold=0.01)
inspect_right_arm(file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded.hdf5")
modify_rewards_and_create_dones(input_path="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded.hdf5", output_path="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr.hdf5")
inspect_right_arm(file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr.hdf5")
shift_actions_with_clipping(input_path="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr.hdf5", output_path="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted.hdf5", k=3)
inspect_right_arm(file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted.hdf5")
truncate_demos_at_k_dones(input_path= "/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted.hdf5", output_path = "/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted_end_cut.hdf5", k = 1)
inspect_right_arm(file="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted_end_cut.hdf5")
switch_to_angle_axis(input_path ="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted_end_cut.hdf5", output_path="/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted_end_cut_angle_axis.hdf5")
inspect_right_arm_angle_axis(file = "/home/qtf5422/Desktop/AIRE/ibrl-docker/data/cube_picking_and_placing_ee/tresholding_cube_nf_floor_cut_angle_axis/dataset_1_norm_gripper_tresholded_wr_shifted_end_cut_angle_axis.hdf5")
