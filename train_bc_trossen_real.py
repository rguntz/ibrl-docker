...
class MainConfig(common_utils.RunConfig):
    ...
    rl_image_size: int = -1
    # log
    save_dir: str = "/home/qtf5422/Desktop/AIRE/DATA/IBRL/run_ee_bc_real_cube_1"
    use_wb: int = 0
    save_per: int = -1
...
def run(cfg: MainConfig, policy):
    ...
    saver = common_utils.TopkSaver(save_dir=cfg.save_dir, topk=1)
    writer = SummaryWriter(log_dir=os.path.join(cfg.save_dir, "tensorboard"))
    stopwatch = common_utils.Stopwatch()
    best_score = 0
    optim_step = 0

    if cfg.training_type == "training":
        print("TRAINING")
        for epoch in range(1, cfg.num_epoch):
            ...
            if epoch % 2 == 0 and epoch > 0:
                save_name = f"checkpoint_step_{epoch}"
    ...
    elif cfg.training_type == "eval":
        print("EVALUATING")
        epoch = 1
        seed = epoch * cfg.num_eval_episode + 1
        scores = evaluate(policy, dataset, seed=seed, num_game=cfg.num_eval_episode, denormalization_path_bc = cfg.denormalization_path_bc, initial_position_file = cfg.initial_position_file)
        score = float(np.mean(scores))
        print("the score is : ", score)
        
        model_name = os.path.basename(cfg.load_model)
        # -> checkpoint_step_2.pt

        results_path = os.path.join(
            cfg.save_dir,
            f"evaluation_results_{model_name}.txt"
        )
        ...
        with open(results_path, "w") as f:
            ...
            f.write(f"Number of Episodes: {cfg.num_eval_episode}\n")


def evaluate(policy, dataset: RobomimicDataset, seed, num_game, denormalization_path_bc, initial_position_file):
    return run_eval_mp(
        dataset.env_params, policy, num_game=num_game, seed=seed, num_proc=1, verbose=False, denormalization_path_bc = denormalization_path_bc, initial_position_file = initial_position_file
    )
