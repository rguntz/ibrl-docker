    ...
    *,
    ...
    while done_episodes < num_episodes:
        ...
        if any(done):
            ...
            # policy.reset(env_ids=terminated_envs) for act
            policy.reset()
...
def print_first_500_next_done(dataset, n=500):
    """
    Print the next.done value for the first n steps of the dataset.

    Args:
        dataset: LeRobotDataset
        n (int): number of steps to print (default: 500)
    """
    n = min(n, len(dataset))

    for i in range(n):
        done = dataset[i]["next.done"].item()
        print(f"step {i:04d} | next.done = {done}")
...
def main(cfg: argparse.Namespace):
    ...
    # Suppose your dataset is called `ds`
    # Get one sample from the dataset
    sample = dataset[0]  # first element, could be any index

    # Now, look at the keys for images
    print(sample.keys())  # to see all features in this sample

    # Access a specific image, e.g., agentview
    agentview_image = sample['observation.images.agentview']
    # print_first_500_next_done(dataset, n=1000)

    # Print its shape
    print("agentview image shape:", agentview_image.shape)

    # You can do the same for other camera images
    left_hand_image = sample['observation.images.robot0_eye_in_left_hand']
    state_debug = sample['observation.state']
    action_debug = sample['action']
    print("left hand camera image shape:", left_hand_image.shape)
    print("state input shape : ", state_debug.shape)
    print("action shape : ", action_debug.shape)
