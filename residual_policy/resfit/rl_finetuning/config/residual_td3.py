...
class ResidualTD3CoffeeConfig(ResidualTD3BoxCleanConfig):
    ...
    base_policy: BasePolicyConfig = field(
        default_factory=lambda: BasePolicyConfig(
            wandb_id="dexmg-bc/xpluwgi8", # here we changed this with our wandb run. 
            wt_type="best",
            wt_version="latest",
        )
    )
...
@dataclass
class ResidualTD3AiireConfig(ResidualTD3BoxCleanConfig):
    task: str = "ECU"

    rl_camera: list[str] = field(
        default_factory=lambda: [
            "observation.images.video",
            "observation.images.video_2",
    ...
    offline_data: OfflineDataConfig = field(
        default_factory=lambda: OfflineDataConfig(
            name="/home/qtf5422/Desktop/AIRE/residual-offpolicy-rl/arm=trossen_dual/task=entire_ecu_assembly",
            num_episodes=1_000,
        )
    )
    base_policy: BasePolicyConfig = field(
...
cs.store(name="residual_td3_aiire_config", node=ResidualTD3AiireConfig)
