...
class QAgent_ibrl(nn.Module):
    ...
    def act(self, obs: dict[str, torch.Tensor], *, eval_mode=False, stddev=0.0, cpu=True) -> torch.Tensor:
        ...
        action = self._act_ibrl( # change the acting method to ibrl
            obs=obs,
            eval_mode=eval_mode,
            stddev=stddev,
            clip=None,
    ...
    def _act_ibrl(
        ...
        *,
        ...
        # Get the residual from the RL policy
        ...
        base_action = obs["observation.base_action"]  # assuming obs, not next_obs
        full_rl_action = torch.clamp(base_action + residual, -1.0, 1.0)
        bc_action = base_action  # since BC action is just the base action

        # Stack full actions for Q evaluation
        ...
        # Select best action index (0 = full_rl_action, 1 = bc_action)
        greedy_action_idx: torch.Tensor = qa.view(bsize, num_action).argmax(1)  # shape [bsize]

        # Now decide what to return: residual if idx==0, else zero
        ...
        if not eval_mode and eps_greedy < 1.0:
            ...
            # For epsilon-greedy exploration: sometimes pick random action
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_residual_action = self._act_ibrl( # replaced with ibrl. 
                obs=next_obs,
            ...
            if self.residual_actor:
                ...
                next_action = torch.clamp(next_obs["observation.base_action"] + next_residual_action, -1.0, 1.0) # we choose this scenario. 
            else:
