...
from resfit.rl_finetuning.off_policy.rl_agnostic.q_agent import QAgent
...
from resfit.rl_finetuning.wrappers.residual_env_wrapper_agnostic import BasePolicyVecEnvWrapper
...
def main(cfg: ResidualTD3DexmgConfig):
    ...
    policy_dir, _ = (
    Path("/home/qtf5422/Desktop/AIRE/residual-offpolicy-rl/bc_run_2026-02-09_13-46-06_dexmg-two-arm-coffee_diffusion/best_step_2000/policy"),
    None,
    )
    ...
    def update_agent_view(im, new_img):
        new_img = preprocess_img(new_img)
        im.set_data(new_img)
    ...
    def preprocess_img(img):
        """
        Converts RL env image to matplotlib-compatible format.
        Handles:
    ...
    cache_dir = OFFLINE_CACHE_DIR / "dea327e7"
    ...
    if len(online_rb) < cfg.algo.learning_starts and not loaded_online_from_cache:
        ...
        plt.ion()
        fig, ax = plt.subplots()
        im = ax.imshow(preprocess_img(obs["observation.images.agentview"]))
        ax.axis("off")
        ...
        while len(online_rb) < cfg.algo.learning_starts:
            ...
            next_obs, reward, terminated, truncated, info = env.step(agent=agent)

            # For plotting : 
            update_agent_view(im, next_obs["observation.images.agentview"])
    ...
    time.sleep(10000) # after the warming. 
    ...
    while global_step <= cfg.algo.total_timesteps:
        ...
        # (4) Updates -------------------------------------------------------
        # ------------------------------------------------------------------
        if global_step % cfg.algo.update_every_n_steps == 0 or global_step == cfg.num_envs: # update_every_n_steps is set to one in the config. 
            i = 0
