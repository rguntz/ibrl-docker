...
from env.trossen_env_delta import Trossen_env
from env.trossen_wrapper import Trossen_wrapper
...
def create_env(
    ...
    # Create the simple trossen wrapper based on the trossen env. 
    ...
    env = Trossen_env(follower_left_ip, follower_right_ip, control_dt, model, max_episode_steps=max_number_steps)

    # Wrap it with the base policy wrapper and the trossen env wrapper. 
    return BasePolicyVecEnvWrapper(
        vec_env=Trossen_wrapper(env),
...
def main(cfg: ResidualTD3DexmgConfig):
    ...
    # TODO fill the path of the policy. 
    policy_dir, _ = (
    Path("/home/qtf5422/Desktop/AIRE/residual-offpolicy-rl/bc_run_2026-02-23_19-15-39_task=entire_ecu_assembly_diffusion/policy_step_72000_val0.0051/policy"),
    None,
    )
    ...
    print("cfg.offline_data.name : ", cfg.offline_data.name)
    ...
    cfg.eval_num_envs = 1 # overwrite to 1 eval env. 
    cfg.eval_num_episodes = 20 # overwrite to 20 evaluations. 
    ...
    online_batch_size = int(cfg.algo.batch_size * (1 - cfg.algo.offline_fraction)) # offline fraction is 0.5. 
    ...
    print("cfg.algo.buffer_size : ", cfg.algo.buffer_size)
    ...
    print("estimated transition : ", estimated_transitions)
    ...
    personal_estimated_offline_transitions = int((RAM_AVAILABLE - 2 - cfg.algo.buffer_size*0.0013)/0.0013)  # ( 0.00014 is the size in giga bytes of a transition)
    ...
    offline_rb = TensorDictPrioritizedReplayBuffer(
        storage=LazyTensorStorage(max_size=personal_estimated_offline_transitions, device="cpu"),
        alpha=alpha,
        beta=beta,
        eps=1e-6,  # Small epsilon added to priorities to prevent zero values
        priority_key="_priority",
    ...
    def _populate_offline_buffer(
        ...
        # Get the max size from the storage to use as a limit
        # Note: LazyTensorStorage has a max_size attribute
        max_capacity = rb.storage.max_size 
        
        for sample in tqdm(loader, desc="Processing offline dataset"):
            # Check if buffer is full BEFORE processing further or at start of loop
            if len(rb) >= max_capacity:
                print(f"\n[INFO] Offline buffer reached capacity ({max_capacity} transitions). Stopping population early.")
                break
            ...
            # ... [existing logic to extract _gt_action, base_action, curr_obs] ...
            # (Keep all your existing extraction logic here)
            ...
            # ... [existing logic for base_action generation] ...
            ...
            if ep_idx in episode_cache:
                ...
                # --- NEW CHECK: Stop immediately if full after adding ---
                if len(rb) >= max_capacity:
                    print(f"\n[INFO] Buffer filled exactly at {transitions} transitions. Breaking loop.")
                    break
                # --------------------------------------------------------

                ## ADDED CODE TO SAVE THE BUFFER ONCE IN A WHILE...
                if transitions % SAVE_EVERY == 0:
                    print(f"Checkpoint: saving buffer after {transitions} transitions...")
                    cache_dir.mkdir(parents=True, exist_ok=True)
                    optimized_replay_buffer_dumps(rb, cache_dir)
                    with open(cache_dir / "user_metadata.json", "w") as f:
                        json.dump(offline_cache_meta, f, indent=2)
                    print(f"Buffer checkpoint saved to {cache_dir}")
            else:
                step_id = 0
        ...
        print(f"Added {transitions} transitions (Final Buffer Size: {len(rb)})")
        return transitions
    ...
    if len(online_rb) < cfg.algo.learning_starts and not loaded_online_from_cache:
        ...
        while len(online_rb) < cfg.algo.learning_starts:
            if cfg.algo.use_base_policy_for_warmup: # prefer doing that. 
                ...
                rand_actions = torch.zeros((cfg.num_envs, action_dim), device=device) # no random actions just for testing the pipeline. 
            else:
            ...
            _add_transitions_to_buffer(
                ...
                online_rb=online_rb, # we add this to the online buffer. 
            )
            ...
            if done :
                ...
                obs = next_obs # roll the next state
    ...
    def _run_critic_warmup(
        ...
        for i in range(cfg.algo.critic_warmup_steps): # the number of critic warmups. 
            ...
            with training_timer.time("gradient_update"):
                metrics = agent.update(batch, stddev=0.0, update_actor=False, bc_batch=None, ref_agent=agent) # update_actor is set to false

            # Update priorities for prioritized experience replay
            if cfg.algo.sampling_strategy == "prioritized_replay" and "_td_errors" in metrics: # means that we sample according to the td error of the buffer, like this 
                # we update the critic more on where the critic was wrong. 
...
python resfit/rl_finetuning/scripts/train_residual_td3_aiire.py     --config-name=residual_td3_aiire_config     algo.prefetch_batches=4     algo.n_step=5     algo.gamma=0.995     algo.learning_starts=10_000     algo.critic_warmup_steps=10_000     algo.num_updates_per_iteration=4     algo.stddev_max=0.025     algo.stddev_min=0.025     algo.buffer_size=21538     agent.actor.action_scale=0.2     agent.actor_lr=1e-6     wandb.project=aiire     wandb.name=resfit     wandb.group=resfit     debug=false
