...
from resfit.rl_finetuning.off_policy.ibrl.q_agent import QAgent_ibrl # new import for IBRL 
...
class LivePlotter:
    def __init__(self, shape):
        """
        shape: (H, W, C) of the image to be plotted
        """
        plt.ion()  # interactive mode on
        self.fig, self.ax = plt.subplots()
        self.im = self.ax.imshow(np.zeros(shape, dtype=np.uint8))
        self.ax.axis("off")
        self.fig.show()

    def update(self, next_obs):
        # Extract image
        img = next_obs["observation.images.agentview"]
        ...
        if img.ndim == 4 and img.shape[0] == 1:
            img = img[0]  # shape now (C, H, W)

        # Convert from CHW to HWC
        if img.shape[0] in [1, 3]:  # channels first
...
def main(cfg: ResidualTD3DexmgConfig):
    ...
    plotter = LivePlotter(shape=(84, 84, 3))
    ...
    env = get_envs(            # base env + new wrapper on top of it to allow the base policy to intervene. 
        env_name=cfg.task,
    ...
    def _populate_offline_buffer(
        ...
        for sample in tqdm(loader, desc="Processing offline dataset"):
            ...
            if use_base_policy_for_base_actions:
                ...
                base_action_scaled = gt_action_scaled # scale the action (because the output of the final policy must be between -1 and 1)
            ...
            if ep_idx in episode_cache:
                ...
                ## ADDED CODE TO SAVE THE BUFFER ONCE IN A WHILE TO AVOID CRASHING. 
    ...
    # We skip the warmup of the critic because IBRL do not do critic warmup. 
    ...
    while global_step <= cfg.algo.total_timesteps:
        ...
        with training_timer.time("env_step"):
            ...
            if cfg.algo.progressive_clipping_steps > 0:
                clip_factor = min(1.0, global_step / cfg.algo.progressive_clipping_steps) # scale for the action output. 
        ...
        # (4) Updates -------------------------------------------------------
        # ------------------------------------------------------------------
        if global_step % cfg.algo.update_every_n_steps == 0 or global_step == cfg.num_envs:      # update_every_n_steps is set to 1 
            i = 0 # how many updates we’ve done in this training iteration.
            actor_update_cadence = cfg.algo.num_updates_per_iteration // cfg.algo.actor_updates_per_iteration  # here num_updates_per_iteration is 4 and actor_updates_per_iteration is 1 so result is 4. 
            ...
            while i < cfg.algo.num_updates_per_iteration:
                ...
                update_actor = (i + 1) % actor_update_cadence == 0 # decide to update the actor or not according to the ratio of critc/actor update. 
