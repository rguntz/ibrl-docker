...
class TimeStep:
    def __init__(self, observation, reward, discount=1.0, step_type=None,
        ...
        self._step_type = step_type  # 'first', 'mid', 'last'
...
class Trossen_env : 
    def __init__(
        ...
        # Initialize keyboard success monitor lazily on first reset
        # (created and started in reset()).
    ...
    def format(self, image: np.ndarray) -> np.ndarray:
            ...
            try:
                ...
                if color_frame:
                    ...
                    if img_resized.ndim == 3 and img_resized.shape[2] == 3:
                        ...
                        images[name] = np.zeros((256, 256, 3), dtype=np.uint8)
                else:
                    images[name] = np.zeros((256, 256, 3), dtype=np.uint8)
            except RuntimeError:
                # Timeout or no frame
    ...
    def get_latest_images(self):
        with self.image_buffer_lock:
            if not self.image_buffer:
                return {name: np.zeros((256, 256, 3), dtype=np.uint8) 
    ...
    def send_cartesian_command(
        ...
        gripper_pos = GRIPPER_MAX * (1 - gripper_pos)
    ...
    def send_cartesian_command_target_init(
        self,
        driver: trossen_arm.TrossenArmDriver,
        target_pose: np.ndarray,
        gripper_pos: float,
    ...
    def move_followers_home(self, home_arm_left = np.zeros(6), home_arm_right = np.zeros(6), gripper_open_left=0.044, gripper_open_right=0.044) : 

        # Home state for arms
        print("home_arm_left : ", home_arm_left)
        print("home_arm_right : ", home_arm_right)
        left_state = np.concatenate([home_arm_left, [gripper_open_left]])
        right_state = np.concatenate([home_arm_right, [gripper_open_right]])
    ...
    def _get_observation(self):
        ...
        # Update current poses for delta action conversion
        self.current_left_pose = follower_left_pose.copy()
        self.current_right_pose = follower_right_pose.copy()
        ...
        # Add reward based on keyboard success state (if monitor exists)
        if self.success_monitor is not None and self.success_monitor.get_success_state():
    ...
    def pose_quat_to_axis_angle(self, pose_7d: np.ndarray) -> np.ndarray:
        """
        Converts a pose [x, y, z, qx, qy, qz, qw] to
        [x, y, z, rx, ry, rz] where (r*) is axis * angle.
        """
        pose_7d = np.asarray(pose_7d)

        rotvec = R.from_quat(pose_7d[3:7]).as_rotvec()

        return np.concatenate([pose_7d[0:3], rotvec])

    def reset(self):
        # Lazily create/start keyboard monitor on first reset
        ...
        # Reset both success and interrupt flags
        self.success_monitor.reset_all_states()
        ...
        # move to the initial position of the dataset : 
        parquet_file = "/home/qtf5422/Desktop/AIRE/residual-offpolicy-rl/arm=trossen_dual/task=entire_ecu_assembly/data/chunk-000/episode_000830.parquet"
        parquet = pd.read_parquet(parquet_file)
        state = parquet["observation.state"]
        initial_state = state[80]
        target_pose_left = self.pose_quat_to_axis_angle(initial_state[0:7])
        target_pose_right = self.pose_quat_to_axis_angle(initial_state[8:15])
        gripper_left = initial_state[7]
        gripper_right = initial_state[15]
        self.send_cartesian_command_target_init(self.follower_left, target_pose = target_pose_left, gripper_pos = gripper_left, robot_id='follower_left')
        self.send_cartesian_command_target_init(self.follower_right, target_pose = target_pose_right, gripper_pos = gripper_right, robot_id='follower_right')
    ...
    def step(self, action):
        """
        Step the environment with delta actions.
         
        Args:
            action: [left_delta_pos(3), left_delta_aa(3), left_gripper(1), 
                    right_delta_pos(3), right_delta_aa(3), right_gripper(1)]
                    Total: 14 elements
        """
        loop_start = time.time()

        # Check if episode is paused - if so, wait until unpaused
        while (self.success_monitor is not None) and self.success_monitor.get_pause_state():
        ...
        # Extract delta actions
        left_delta_pos = action[:3]
        left_delta_aa = action[3:6]
        left_gripper = action[6]
        
        right_delta_pos = action[7:10]
        right_delta_aa = action[10:13]
        right_gripper = action[13]
        
        # Rotation: need to compose rotations properly
        # Convert current and delta angle-axis to rotation matrices, compose them, then back to angle-axis

        # NOTE:
        # left_delta_aa / right_delta_aa represent a LOCAL (body-frame) rotation delta.
        # They were computed as:
        #   R_delta = R_target @ R_current.T
        # which gives the rotation needed to go from the current orientation to the
        # target, expressed in the current (body) frame.
        #
        # In SciPy, rotation composition follows:
        #   R_new = R1 * R2  ->  apply R2 first, then R1
        #
        # Therefore, to correctly apply a LOCAL delta, we must left-multiply:
        #   R_new = R_delta * R_current
        # (and NOT R_current * R_delta, which would correspond to a global delta)
        # Get the latest observation : 
        self._get_observation()

        # Convert delta to absolute actions
        # Position: simple addition is correct
        left_absolute_pos = self.current_left_pose[:3] + left_delta_pos
        right_absolute_pos = self.current_right_pose[:3] + right_delta_pos
        
        left_current_rot = R.from_rotvec(self.current_left_pose[3:])
        left_delta_rot = R.from_rotvec(left_delta_aa)
        left_new_rot = left_delta_rot * left_current_rot
        left_absolute_aa = left_new_rot.as_rotvec()

        right_current_rot = R.from_rotvec(self.current_right_pose[3:])
        right_delta_rot = R.from_rotvec(right_delta_aa)
        right_new_rot = right_delta_rot * right_current_rot
        right_absolute_aa = right_new_rot.as_rotvec()
        
        left_absolute_pose = np.concatenate([left_absolute_pos, left_absolute_aa])
        right_absolute_pose = np.concatenate([right_absolute_pos, right_absolute_aa])

        # Send absolute commands
        self.send_cartesian_command(self.follower_left, left_absolute_pose, left_gripper, robot_id='follower_left')
        self.send_cartesian_command(self.follower_right, right_absolute_pose, right_gripper, robot_id='follower_right')
        ...
        # Reward as numpy
        reward = np.array([observation['reward']], dtype=np.float32)

        # Interrupt flag (external stop)
        interrupted = (
            self.success_monitor.get_interrupt_state()
        ...
        # Success condition
        success = (
            self.success_monitor.get_success_state()
            if self.success_monitor is not None
            else False
        )
        ...
        # Termination (true terminal state)
        terminated = success or interrupted
