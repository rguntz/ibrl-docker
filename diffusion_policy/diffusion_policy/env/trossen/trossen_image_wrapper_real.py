...
class TrossenImageWrapper(gym.Env):
    ...
    def reset(self) -> Dict[str, np.ndarray]:
        ...
        if not REAL_SETUP : 
            file = "/home/qtf5422/Desktop/AIRE/ibrl-docker/diffusion_policy/diffusion_policy/data/trossen/real/transfer_rectangle/dataset_tresholded_gripper_normed_reward_dones_cut_end_image_transpose.hdf5"
            self.action_dim = 14
        with h5py.File(file, "r") as f:
            ...
            # for i in range(1) : # only to replay the actions, we only need the demo 0. 
                ...
                for j in range(1) : 
                    if not REAL_SETUP : # simulation : use the angle axis
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, Dict]:
        ...
        # action[:7] = self.mean[:7] # keep the left robot immobile => only block the left arm for single arm manipulation. 
