...
print("importing the lib")
from diffusion_policy.env_runner.trossen_env import Trossen_env 
print("finished importing the lib")
...
class EvalProcess:
    ...
    def create_env(self):
        ...
        from diffusion_policy.env.trossen.trossen_image_wrapper_real import TrossenImageWrapper
        from diffusion_policy.gym_util.multistep_wrapper_real import MultiStepWrapper
        ...
        trossen_env = Trossen_env(follower_left_ip, follower_right_ip, control_dt, model, max_episode_steps=max_number_steps)
...
class MultiprocessTrossenImageRunner(BaseImageRunner):
    ...
    def denormalize_gripper(self, action: np.ndarray) -> np.ndarray:
        ...
        Gripper indices in 14D action: [6, 13]
        """
        action = action.copy()
    ...
    def undo_transform_action(self, action: np.ndarray) -> np.ndarray:
        ...
        action_final = self.denormalize_gripper(action_14d)
